...
def get_favorite_by_user_id(db:Session, user_id:str, user: Optional[User]) -> list[Work]:
    ...
    for favorite in favorite_by_user_id_list:
        ...
        if user is not None:
            work.is_favorite = db.query(models.Favorite).filter(models.Favorite.work_id == favorite.work_id, models.Favorite.user_id == user.id).first() is not None
        else:
            work.is_favorite = False
        work.favorite_count = db.query(models.Favorite).filter(models.Favorite.work_id == favorite_work.id).count()
