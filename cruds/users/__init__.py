...
def get_user_by_id(db: Session, user_id: str) -> UserSchema:
    ...
    if user_orm is None:
        raise HTTPException(status_code=404, detail="The tag is not exist")
    user = UserSchema.from_orm(user_orm)
    return user


def get_users(
    db: Session, limit: int = 30, oldest_user_id: str = None, newest_user_id: str = None
) -> list[UserSchema]:
    user_list = []
    ...
    if oldest_user_id:
        oldest_user = (
            db.query(models.User).filter(models.User.id == oldest_user_id).first()
        )
        if oldest_user is None:
            raise HTTPException(status_code=400, detail="oldest user are not exist")
        users = users.filter(models.User.updated_at > oldest_user.updated_at)
    if newest_user_id:
        newest_user = (
            db.query(models.User).filter(models.User.id == newest_user_id).first()
        )
        if newest_user is None:
            raise HTTPException(status_code=400, detail="newest user are not exist")
...
def change_user_info(
    ...
    user_orm.display_name = (
        user_orm.display_name if display_name is None else display_name
    )
