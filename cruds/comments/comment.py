...
def create_comment(
    ...
    if reply_at:
        comment_orm = (
            db.query(models.Comment).filter(models.Comment.id == reply_at).first()
        )
        if comment_orm is None:
            raise HTTPException(status_code=404, detail="reply_at is invalid")

        if comment_orm.reply_at:
            raise HTTPException(status_code=400, detail="Can't reply to this reply_at")
...
def get_comments_by_work_id(
    ...
    if work_orm is None:
        raise HTTPException(status_code=404, detail="work_id is invalid")
    ...
    if auth:
        comments_orm = comments_orm.filter(
            models.Comment.visibility != models.Visibility.draft
        )
    else:
        comments_orm = comments_orm.filter(
            models.Comment.visibility == models.Visibility.public
        )

    if oldest_comment_id:
        oldest_comment = (
            db.query(models.Comment)
            .filter(models.Comment.id == oldest_comment_id)
            .first()
        )
        if oldest_comment is None:
            raise HTTPException(status_code=400, detail="oldest comment is not found")
        comments_orm = comments_orm.filter(
            models.Comment.created_at > oldest_comment.created_at
        )
    if newest_comment_id:
        newest_comment = (
            db.query(models.Comment)
            .filter(models.Comment.id == newest_comment_id)
            .first()
        )
        if newest_comment is None:
            raise HTTPException(status_code=400, detail="newest comment is not found")
        comments_orm = comments_orm.filter(
            models.Comment.created_at < newest_comment.created_at
        )
    ...
    for item in comments_orm:
        reply_orm = (
            db.query(models.Comment)
            .filter(models.Comment.work_id == work_id)
            .filter(models.Comment.reply_at == item.id)
        )
        if auth:
            reply_orm = reply_orm.filter(
                models.Comment.visibility != models.Visibility.draft
            )
        else:
            reply_orm = reply_orm.filter(
...
def get_reply_comments_by_comment_id(
    db: Session,
    comment_id: str,
    work_id: str,
    limit: int = 30,
    ...
    if comment_orm is None:
        raise HTTPException(status_code=404, detail="comment_id is invalid")

    comments_orm = (
        db.query(models.Comment)
        .filter(models.Comment.reply_at == comment_id)
...
def delete_by_comment_id(
    db: Session, comment_id: str = None, work_id: str = None, user_id: str = None
) -> DeleteStatus:
    comment = (
        db.query(models.Comment)
        .filter(models.Comment.work_id == work_id)
    ...
    if comment.reply_at is None:
        db.query(models.Comment).filter(models.Comment.work_id == work_id).filter(
            models.Comment.reply_at == comment_id
        ).delete()
