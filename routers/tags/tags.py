...
from schemas.tag import PostTag, GetTag, TagResponsStatus, PutTag
...
async def get_all_tags(community_id: str = None, limit: int = 30, offset_id: str = None, db: Session = Depends(get_db)):
...
async def put_tag_by_id(tag_id: str, payload: PutTag, db: Session = Depends(get_db), user: User = Depends(GetCurrentUser())):
...
async def delete_tag(tag_id: str, db: Session = Depends(get_db), user: User = Depends(GetCurrentUser())):
